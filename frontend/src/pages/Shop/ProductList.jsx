...
const ProductList = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            const response = await ProductServie.getAllProducts();
            ...
            <div className="flex flex-col md:flex-row flex-wrap md:justify-between items-center space-y-3 mb-8">
                ...
                <div className="flex flex-row justify-start md:items-center md:gap-8 gap-4 flex-wrap">
                    {categories.map((category, index) => {
                        return (
                            <button key={index} className={`${selectedCategory === category ? "active" : ""} px-4 py-2 rounded-full`} onClick={() => filterItems}><p className="capitalize">{category}</p></button>
                        );
                    })}
                </div>
                {/* Sort Options */}
                ...
                <div className="flex justify-center my-8 flex-wrap gap-2">
                    {
                        Array.from({ length: Math.ceil(filteredItems.length / itemsPerPage), }).map((_, index) => (
                            <button key={index} className={`mx-1 px-3 py-1 rounded-full ${currentPage === index + 1 ? "bg-red text-white" : "bg-grey"} `} onClick={() => paginate(index + 1)}>
                                {index + 1}
                            </button>
                        ))}
                </div>
